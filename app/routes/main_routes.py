...
def api_save_config():
    ...
    try:
        ...
        pass
    # Range constraints: (min_value, allow_zero)
    _float_min = {
        "battery_capacity_kwh": (0, False),   # > 0
        "home_radius_m":        (0, False),   # > 0
        "poll_interval":        (0, False),   # > 0 (also in ints)
        "price_per_kwh_home":   (0, True),    # >= 0
        "price_per_kwh_ac":     (0, True),    # >= 0
        "price_per_kwh_dc":     (0, True),    # >= 0
        "dc_threshold_kw":      (0, True),    # >= 0
        "entsoe_ac_markup":     (0, True),    # >= 0
        "entsoe_dc_markup":     (0, True),    # >= 0
    }
    for key in accepted_keys:
        if key in data:
            ...
            if key in floats and v != "" and v is not None:
                try:
                    v = float(v)
                except (ValueError, TypeError):
                    return jsonify({"ok": False, "error": f"Ungültiger Zahlenwert für '{key}'"}), 400
                if key in _float_min:
                    mn, allow_zero = _float_min[key]
                    if (not allow_zero and v <= mn) or (allow_zero and v < mn):
                        limit = f"> {mn}" if not allow_zero else f">= {mn}"
                        return jsonify({"ok": False, "error": f"'{key}' muss {limit} sein"}), 400
            elif key in ints and v != "" and v is not None:
                try:
                    v = int(v)
                except (ValueError, TypeError):
                    return jsonify({"ok": False, "error": f"Ungültiger Ganzzahlwert für '{key}'"}), 400
                if key in _float_min:
                    mn, allow_zero = _float_min[key]
                    if (not allow_zero and v <= mn) or (allow_zero and v < mn):
                        limit = f"> {mn}" if not allow_zero else f">= {mn}"
