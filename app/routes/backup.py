...
@require_login
def api_backup_list():
    from server import create_backup, restore_backup, BACKUP_DIR, parse_cron_next
...
def api_backup_upload():
    from server import create_backup, restore_backup, BACKUP_DIR, parse_cron_next, _BACKUP_MAX_UPLOAD_BYTES
...
@require_login
def api_backup_cron():
    from server import create_backup, restore_backup, BACKUP_DIR, parse_cron_next, schedule_backup, save_config
    if not has_permission(_current_user(), "backup:create"):
        return jsonify({"error": "Keine Berechtigung: backup:create"}), 403
    import server as _server
    cron=request.json.get("cron","").strip()
    cfg=load_config(); cfg["backup_cron"]=cron; save_config(cfg)
    if _server._backup_timer: _server._backup_timer.cancel()
    if cron:
        schedule_backup()
        secs=parse_cron_next(cron)
